...
The Stabilization Module may be used in a [](@mffs:projector) to crystallise the field into a solid form. Instead of
...
in combination with the [](@mffs:custom_module) feature of a Projector, its conceivable applications are virtually
limitless.

For example, a player could create a structure that was automatically repaired whenever a [Creeper](@creeper) tried to
blow it up.
Adding [Speed Modules](@mffs:speed_module) into a Projector allows multiple blocks to be stabilized at once.
...
type of each block it analyses. As a result, the stabilization process for the [](@mffs:custom_module) does not work in
the same way.
