...
To configure the Biometric Identifier to recognise a player, open its GUI screen and place an [](@mffs:id_card) into
the top-left item slot labeled Rights. If the [](@mffs:id_card) is blank and hasn't already been assigned, you can
...
complete the process. To set yourself as the master of a Biometric Identifier, place an [](@mffs:id_card) assigned to
...
[Identification Cards](mffs:id_card) can't be removed from the Identifier while it's active.
