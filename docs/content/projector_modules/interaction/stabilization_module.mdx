...
in combination with the [](@mffs:custom_mode) feature of a Projector, its conceivable applications are virtually
limitless.
...
type of each block it analyses. As a result, the stabilization process for the [](@mffs:custom_mode) does not work in
the same way.
